...
void menuStudentManagement(){
	...
	printf("[0] save file and come back\n");
	printf("==========================================\n");
};
...
void showListStudent(struct Student *studentList, int begin ,int numberOfStudent1){
	...
	if(n != 0 ){
		...
		printf("|==========|=========================|==============================|===============|==========|\n");
		for(i = begin; i < n; i++){
			printf("|%-10s|%-25s|%-30s|%-15s|%-10d|\n",
					studentList[i].studentId,studentList[i].nameStudent,studentList[i].email,studentList[i].phoneNumberStudent,studentList[i].numberOfClassroom);
			printf("|==========|=========================|==============================|===============|==========|\n");
		}
...
void addNewStudent(struct Student *studentList2, int *numberOfStudent2, int check ){
	...
	checkc = 1;
	studentList2[n].numberOfClassroom = 0; // cho mac dinh
...
void searchStudent(struct Student *studentList5, int numberOfStudent5){
	...
	if( n != 0){
		...
		for(i =0 ; i< n; i++){
			if(strstr( studentList5[i].nameStudent, name) != 0){
				serial+=1;
				if(serial == 1){
...
void addNewClassroom(struct Classroom *classroomList2, int *numberOfClass2){
	...
	*numberOfClass2 +=1; // tang so luong lop len 1 nek
	resetCmd();
};
...
void showFullClass(struct Classroom *classroomList5, int numberOfClass5, struct Student *StudentList1 , int numberOfStudent1){
	if(numberOfClass5 != 0 ){
		int i, n=numberOfClass5, checkS = 0, check8 = 0;
		char idCheck[10]; printf("enter classroom'id to check: "); scanf("%s",idCheck);
		for(i =0; i < n; i++){
			if( strcmp(classroomList5[i].classroomId, idCheck) == 0){
				checkS +=1;
				if(checkS == 1){
					printf("+================+\n");
					printf("|classroom here! |\n");
				}
				printf("+========================================+\n");
				printf("|id: %-37s|\n", classroomList5[i].classroomId);
				printf("|name: %-35s|\n", classroomList5[i].classroomName);
				printf("|majoy: %-34s|\n",classroomList5[i].majoy);
				printf("|size: %-35d|\n",classroomList5[i].numberOfStudent);
				printf("|status: %-33s|\n",classroomList5[i].status);
				printf("+========================================+\n\n");
			    
			 	printf("+============================+\n");
			 	printf("|[1] add new student to class|\n");
			 	printf("|[2] delete student of class |\n");
			 	printf("|[0] no action\n             |\n");
			 	printf("+============================+\n");
			 	int choiceADN = inputChoice(0,2);
			break;
			}
		}
		if(checkS == 0){
...
// quan ly lop hoc
void classManagement(struct Classroom *classrooms, int *numberOfClass, struct Student *studentList, int *numberOfStudent){
	int t3 = 1;
	while(t3){
		menuClassManagement();
		...
		switch(choice5){
			...
			case 5:{
				system("cls");
				showFullClass(classrooms, *numberOfClass, studentList, *numberOfStudent);
				break;
			}
			case 0:{
				system("cls");
...
void deleteTeacher(struct Teacher *teachers4, int *countTeacher4){
	int i, j, check1 = 0, n = *countTeacher4;
	char idCheck[10]; printf("enter teacher'id to check: ");
	scanf("%s",&idCheck);
	for(i = 0; i < n; i++){
		if(strcmp(idCheck, teachers4[i].teacherId) == 0){
			check1 =1;
			int check2;
			printf("do you want delete teacher(1-yes/0-no)!\n"); scanf("%d",&check2);
			if(check2 == 1){
				for(j = i; j < n; j++){
					teachers4[j] = teachers4[j+1];
				}
				*countTeacher4 -= 1;
				printf("delete successful!\n");
			}else {
				printf("you don't delete that!\n");
			}
			break;
		}
	}
...
void searchTeacher(struct Teacher *teachers5, int countTeacher5){
	char nameCheck[25];
	int count = 0;
	int i, n = countTeacher5;
	printf("enter name to search and check: "); fflush(stdin); gets(nameCheck);
	for(i = 0; i < n; i++){
		if(strstr(teachers5[i].teacherName, nameCheck) > 0){
			count++;
			if(count == 1){
				printf("found that success!\n\n"); // neu thay thi chi in ra 1 lan
				printf("+===============+\n");
				printf("|List teacher   |\n");
			}
			printf("+==============================+\n");
			printf("|%-30s|\n", teachers5[i].teacherName);
		}
	}
	if(count == 0){
		printf("not found that!\n");
	}else {
		printf("+==============================+\n");
	}
	resetCmd();
...
void teacherManagement(struct Teacher *teachers, int *countTeacher){
	...
	while(t5){
		...
		switch(choice6){
			...
			case 4:{
				system("cls");
				deleteTeacher(teachers, &*countTeacher);
				break;
			}
			case 5:{
				system("cls");
				searchTeacher(teachers, *countTeacher);
				break;
			}
			case 0:{
				system("cls"); 
